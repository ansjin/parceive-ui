...
-- Group calls by FUNCTION_ID and CALLER so we don't have to do it in javascript
DROP TABLE IF EXISTS GROUPED_CALL_TABLE;
CREATE TABLE "GROUPED_CALL_TABLE"(ID INT PRIMARY KEY NOT NULL, FUNCTION_ID INT NOT NULL,CALLER CHAR(50),COUNT INT NOT NULL,PARENT INT);
INSERT INTO GROUPED_CALL_TABLE SELECT ROWID, FUNCTION_ID, CALLER, COUNT(*), NULL FROM CALL_TABLE GROUP BY FUNCTION_ID, CALLER;

-- Add CALL_GROUP, CALLS_OTHER and DURATION propertie to CALL_TABLE
ALTER TABLE CALL_TABLE RENAME TO TMP;
CREATE TABLE "CALL_TABLE"(ID CHAR(50) PRIMARY KEY NOT NULL,PROCESS_ID INT NOT NULL,THREAD_ID INT NOT NULL,FUNCTION_ID INT NOT NULL,INSTRUCTION_ID INT NOT NULL,START_TIME CHAR(12),END_TIME CHAR(12),CALLER CHAR(50),CALL_GROUP INT, CALLS_OTHER INTEGER, DURATION INTEGER);
INSERT INTO CALL_TABLE SELECT ID, PROCESS_ID, THREAD_ID, FUNCTION_ID, INSTRUCTION_ID, START_TIME, END_TIME, CALLER, (SELECT g.ID FROM GROUPED_CALL_TABLE g WHERE c.FUNCTION_ID = g.FUNCTION_ID AND c.CALLER = g.CALLER) AS CALL_GROUP,(SELECT COUNT(t.ID) FROM TMP t WHERE t.CALLER = c.ID ) AS CALLS_OTHER, END_TIME - START_TIME AS DURATION FROM TMP c;
DROP TABLE TMP;

-- Fill in PARENT for GROUPED_CALL_TABLE
UPDATE GROUPED_CALL_TABLE SET PARENT=(SELECT c.CALL_GROUP FROM CALL_TABLE c WHERE c.ID = GROUPED_CALL_TABLE.CALLER);
...
CREATE INDEX IF NOT EXISTS CALL_TABLE_CALL_GROUP ON CALL_TABLE(CALL_GROUP);
